...
### Verify permissions and observe expected failures

For all scenarios, you should observe:
- **GET and POST requests succeed** because the managed identities have `Sample.Read` and `Sample.Write` permissions
  - Successful requests return details about the JWT token that was used to authenticate the request
- **DELETE requests fail with 401 Unauthorized** because none of the managed identities have `Sample.Delete` permission
