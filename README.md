...
An Azure Developer CLI (`azd`) template using Bicep that shows how to call Azure API Management (APIM) from Azure Functions and Logic Apps with managed identity authentication. It also demonstrates how one APIM API can call another using managed identity, enabling secure, secret-free authentication between Azure services.
